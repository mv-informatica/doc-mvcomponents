    ...
    var Grid = FC.Grid = Class.extend(ListenerMixin, {
      ...
      dayMousedown: function(ev) {
        ...
        if (view.opt('selectable')) {
          this.daySelectListener.startInteraction(ev, {distance: view.opt('selectMinDistance')});
        }
      },
      dayTouchStart: function(ev) {
        ...
        this.dayClickListener.startInteraction(ev);
        if (view.opt('selectable')) {
          this.daySelectListener.startInteraction(ev, {delay: selectLongPressDelay});
        }
      },
    ...
    Grid.mixin({
      ...
      handleSegTouchStart: function(seg, ev) {
        ...
        if (!isResizing && (isDraggable || isResizable)) {
          ...
          dragListener.startInteraction(ev, {delay: isSelected ? 0 : eventLongPressDelay});
        }
      },
      ...
      renderNumberCellHtml: function(date) {
        ...
        if (view.cellWeekNumbersVisible && (date.day() == weekCalcFirstDoW)) {
          html += view.buildGotoAnchorHtml({
            date: date,
            type: 'week'
          }, {'class': 'fc-week-number'}, date.format('w'));
        }
        if (isDayNumberVisible) {
          html += view.buildGotoAnchorHtml(date, {'class': 'fc-day-number'}, date.date());
        }
        html += '</td>';
        return html;
      },
